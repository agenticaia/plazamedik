...
const Navigation = () => {
  ...
  const menuItems = [{
    ...
    label: "Várices",
    path: "/medias-para-varices"
  }, {
    label: "Trabajo de Pie",
    path: "/trabajo-de-pie"
  }, {
    label: "Piel Sensible",
    path: "/piel-sensible"
  ...
  return <nav className="sticky top-0 z-50 bg-card/95 backdrop-blur-sm border-b border-border shadow-soft">
...
Plazamedik<div className="h-8 w-8 rounded-full bg-primary flex items-center justify-center">
          ...
          <div className="hidden lg:flex items-center gap-6">
            {menuItems.map(item => <Link key={item.path} to={item.path} className="text-sm font-medium text-foreground/80 hover:text-primary transition-colors">
                {item.label}
              </Link>)}
            <Button asChild size="sm" className="bg-whatsapp-green hover:bg-whatsapp-green/90 text-white transition-all">
              <a href={getWhatsAppLink("", "Hola, necesito asesoría sobre medias de compresión")} target="_blank" rel="noopener noreferrer">
                <MessageCircle className="w-4 h-4 mr-2" />
                WhatsApp
              </a>
            </Button>
            {user ? <Button variant="outline" size="sm" onClick={() => signOut()}>
        ...
        {isOpen && <div className="lg:hidden py-4 border-t border-border">
            <div className="flex flex-col gap-4">
              {menuItems.map(item => <Link key={item.path} to={item.path} onClick={() => setIsOpen(false)} className="text-sm font-medium text-foreground/80 hover:text-primary transition-colors py-2">
                  {item.label}
                </Link>)}
            ...
            setIsOpen(false);
                  ...
                  <LogOut className="w-4 h-4 mr-2" />
                  Salir
                </Button> : <Button asChild variant="outline" className="w-full">
