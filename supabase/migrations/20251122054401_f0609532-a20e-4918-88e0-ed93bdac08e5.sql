-- Fix: Update trigger to create purchase_order_items when auto-creating PO at reorder point
-- This ensures POs have proper items and total_cost tracking

CREATE OR REPLACE FUNCTION public.auto_create_po_at_reorder_point()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  THEN
    ...
    -- ✅ CRITICAL FIX: Create purchase_order_item entry
...
-- Re-create trigger (just to ensure it's using the updated function)
